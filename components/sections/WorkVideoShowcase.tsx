    ...
    <section className="mb-16" aria-label="Video showcases">
      <div className="grid gap-5 md:grid-cols-2">
        {videoCards.map((video) => {
          ...
          const card = (
            <div className="group relative aspect-video overflow-hidden rounded-xl border border-white/[0.08] bg-white/[0.03]">
              {videoId ? (
                ...
                <div className="flex h-full items-center justify-center px-6 text-center font-sans text-sm text-white/40">
          ...
          if (!video.url) {
            return (
              <div key={video.title} aria-label={`${video.title}; URL not yet provided`}>
                {card}
              </div>
            );
          }
